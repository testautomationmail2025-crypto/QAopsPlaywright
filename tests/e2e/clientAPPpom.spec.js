...
for (const data of dataset) {
  test(`@web E2E automation test - ${data.productName}`, async ({ page }) => {
    ...
    await cartPage.cartCheckoutBtn(data.productName);
...
customtest("E2E automation test", async ({ page, testDataForOrder }) => {
  const poManager = new POManager(page);
  const loginPage = poManager.getLoginPage();
  const dbPage = poManager.getDashboardPage();
  const cartPage = poManager.getCartPage();
  ...
  await cartPage.cartCheckoutBtn(testDataForOrder.productName);
